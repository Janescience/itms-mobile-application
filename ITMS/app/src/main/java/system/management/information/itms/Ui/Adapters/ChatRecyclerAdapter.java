...
public class ChatRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private void configureMyChatViewHolder(MyChatViewHolder myChatViewHolder, int position) {
        ...
        String alphabet = chat.profileImage;
                ;
        mContext = myChatViewHolder.txtUserAlphabet.getContext();

        myChatViewHolder.txtChatMessage.setText(chat.message);
        Picasso.with(mContext).load(Uri.parse(alphabet)).into(myChatViewHolder.txtUserAlphabet);
    }

    private void configureOtherChatViewHolder(OtherChatViewHolder otherChatViewHolder, int position) {
        Chat chat = mChats.get(position);
        mContext = otherChatViewHolder.txtUserAlphabet.getContext();

        String alphabet = chat.profileImage;

        otherChatViewHolder.txtChatMessage.setText(chat.message);
        Picasso.with(mContext).load(Uri.parse(alphabet)).into(otherChatViewHolder.txtUserAlphabet);
    }
